...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'room_members_payment_info_length'
      and conrelid = 'public.room_members'::regclass
  ) then
    alter table public.room_members
      add constraint room_members_payment_info_length
      check (payment_info is null or char_length(payment_info) <= 500);
  end if;
end;
$$;
...
drop function if exists public.merge_room_members(uuid, uuid, uuid);
